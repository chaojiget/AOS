    ...
    def run_task(self, instruction: str):
        ...
        with self.tracer.start_as_current_span("task_execution") as span:
            ...
            if "git" in instruction.lower():
                ...
                self.logger.info(
                    f"Observation: Retrieved git log ({len(output)} chars)."
                )
            ...
            self.logger.info(
                f"Introspection: Current Context Entropy = {token_count} tokens"
            )
            ...
            if token_count > DEMO_THRESHOLD:
                self.logger.warning(
                    f"CRITICAL: Entropy ({token_count}) exceeded safety limit ({DEMO_THRESHOLD}). Initiating RESET."
                )
