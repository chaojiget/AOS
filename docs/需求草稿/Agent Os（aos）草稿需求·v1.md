...
- 可在聊天页提交任务，Agent 通过 MCP 工具完成 1 次外部行动，**实时可视化**过程，并产生**FINAL_ANSWER**；
...
- **存储**：Drizzle ORM + SQLite（chats / runs / events / skills / memories / mcp_configs / evals）；
...
- `DELETE /api/mcp/:id` 删除 MCP 服务配置并返回剩余列表
...
- **mcp_configs**：`id, name, base_url, auth_json, created_at, enabled`
- **evals**：`id, run_id, label(success|fail|partial), notes, checker, created_at`
- **ab_tests**：`id, run_id, variant, notes`
...
```json
{
  ...
  "execute": {
    "tool": "mcp.playwright.extract",
    "args_map": { "url": "$.url", "selectors": "$.selectors" }
  },
...
3. **审核**：人审 + 小样本回放评测（win_rate ≥ 60% 启用）；
